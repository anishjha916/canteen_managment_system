...
?>
    ...
    <div class="container table-responsive" style="margin-left: 8%;">
        <h1 id="table-name" class="shadow rounded border text-center text-uppercase ">Master table</h1>

        <div id="add_btn" class="shadow rounded mt-2 d-flex justify-content-end">

            <button type="button" class="border-0 shadow m-1 btn btn-primary" data-toggle="modal" data-target="#myModal"> Add Content to Master </button>
        </div>

        <!-- <div id="record-contant"> -->
        <table id="myTable" class="table-hover mt-5 table border shadow table-bordered  bg-light">
        <thead><tr><th>S.NO.</th><th>PRODUCT NAME</th><th>PRODUCT PRICE </th><th>CATEGORY NAME</th><th>UNIT NAME</th><th>Edit Action </th>    <th>Delete Action</th></tr></thead><tbody id="table_body">
            </tbody>
    ...
    function load_values2() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    var optn_body = '<option disabled selected>Price</option>';
                    for (var a = 0; a < response.length; a++) {
                        var value = response[a].Price;
                        // alert(value);
                        optn_body = optn_body + '<option value="' + value + '">' + value + '</option>';
                    }
                    // $("#select").html(optn_body);
...
$('#fifth_input').on('focus', function(){
    ...
    function getMasterDetails(masterID) {
        ...
        $.post("../master_backend.php", {
            ...
            $('#update_third_input').val(master.Price);
...
// });
    ...
    function updateMasterDetails() {
        ...
        var update_Price = $('#update_third_input').val();
        ...
        //     return false;
        // } else if (update_Price.trim() == '') {
        //     $('#update_tax_select_alert').html('Please enter Price');
        //     return false;

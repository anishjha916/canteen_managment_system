...
if (isset($_POST['submit']) && isset($_POST['first_input_value']) && isset($_POST['sec_input_value']) && isset($_POST['third_input_value']) && isset($_POST['fourth_input_value'])) {
    ...
    if ($_POST['len2'] != $_POST['num2']) {
        $date = date('d-m-y');
        $tax_insert_query =  "INSERT INTO `price_table`(`Price`, `added_on`) VALUES('$third_input_value', '$date')";
    ...
    $sql3 = "SELECT `id` FROM price_table WHERE `Price` = '$third_input_value'";
    ...
    $query = "INSERT INTO `master_table`(`product_id`, `category_id`, `tax_id`, `unit_id`) VALUES ($row1[id], '$row2[id]', '$row3[id]', '$row4[id]')";
...
if (isset($_POST['readrecord'])) {
   ...
   $query =  "SELECT master_table.id, product_table.product_name, category_table.category_name, price_table.Price, unit_table.unit_name from master_table INNER JOIN product_table ON product_table.id = master_table.product_id INNER JOIN category_table ON category_table.id = master_table.category_id INNER JOIN price_table ON price_table.id = master_table.tax_id INNER JOIN unit_table ON unit_table.id = master_table.unit_id ORDER BY master_table.id DESC";
...
if (isset($_POST['display2'])) {
    $sql = "select `Price` from price_table";
    $result = mysqli_query($conn, $sql);
    $result_arr = array();
    while ($row = mysqli_fetch_array($result)) {
        $result_arr[] = $row;
...
if (isset($_POST['get_string'])) {
    ...
    $get_insert_query = "SELECT product_table.product_name, category_table.category_name, price_table.Price, unit_table.unit_name FROM master_table INNER JOIN product_table ON product_table.id = master_table.product_id INNER JOIN category_table ON category_table.id = master_table.category_id INNER JOIN price_table ON price_table.id = master_table.tax_id INNER JOIN unit_table ON unit_table.id = master_table.unit_id WHERE product_table.id = '$response[product_id]' AND category_table.id = '$response[category_id]' AND price_table.id = '$response[tax_id]' AND unit_table.id = '$response[unit_id]'";
...
if (isset($_POST['update_string'])) {
    ...
    $update_tax_name = $_POST['update_Price'];
...
// UPDATE master_table, product_table, category_table, price_table, unit_table SET product_table.product_name = 'aalu gunda', category_table.category_name = 'street food', price_table.Price = '33', unit_table.unit_name = 'pcs' WHERE master_table.product_id = 66 AND product_table.id = 38 AND category_table.id = 20 AND price_table.id = 16 AND unit_table.id = 12;
